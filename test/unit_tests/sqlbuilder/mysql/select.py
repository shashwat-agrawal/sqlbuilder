from sqlbuilder.mysql.select import Select
import unittest
...
class TestCase(unittest.TestCase):
    def test_all(self):
        ...
        self.assertEqual(sql.to_sql_and_binds(), ('SELECT XYZ WHERE xyz = %s', [1]))
        ...
        self.assertEqual(sql.to_sql_and_binds(), ('SELECT BAR FROM (SELECT NESTED ABC FROM ABC_TABLE WHERE FOO IN (%s,%s)) AS BAR_TABLE WHERE FOO_BAR = %s', [1, 2, 3]))
